...
@section ('title', trans('labels.backend.access.users.business_management') . ' | ' . trans('labels.backend.access.users.business_deleted'))
...
<div class="box box-success">
    <div class="box-header with-border">
        <h3 class="box-title">{{ trans('labels.backend.access.users.business_deleted') }}</h3>
...
<script>
    $(function () {
        var businessDel = $('#users-table').DataTable({
        dom: 'lfrtip',
                processing: false,
                serverSide: true,
                autoWidth: false,
                ...
                url: '{{ route("admin.access.user.get") }}',
                        type: 'post',
                        data: {status: false, trashed: true, role: 2},
...
//                            location.reload();
    ...
    businessDel.on('order.dt search.dt', function () {
        businessDel.column(0, {search: 'applied', order: 'applied'}).nodes().each(function (cell, i) {
            cell.innerHTML = i + 1;
        });
    }).draw();
