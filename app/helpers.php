...
//        $src = imagecreatefromjpeg($image);
        ...
        if($extension=='png'  ){ $src = imagecreatefrompng($image); }
        elseif($extension=='PNG'){ $src = imagecreatefrompng($image); }
        elseif($extension=='jpg'){ $src = imagecreatefromjpeg($image); }          
        elseif($extension=='JPG'){ $src = imagecreatefromjpeg($image); }       
        elseif($extension=='jpeg'){ $src = imagecreatefromjpeg($image); }
        elseif($extension=='JPEG'){ $src = imagecreatefromjpeg($image); } 
        elseif($extension=='pjpeg'){ $src = imagecreatefrompjpeg($image); }  
        elseif($extension=='gif'){ $src = imagecreatefromgif($image); }
        elseif($extension=='GIF'){ $src = imagecreatefromgif($image); }   
        elseif($extension=='pdf'){ $src = imagecreatefrompdf($image); }        
        elseif($extension=='PDF'){ $src = imagecreatefrompdf($image); } 
        elseif($extension=='tif'){ $src = imagecreatefromtif($image); }        
        elseif($extension=='tiff'){ $src = imagecreatefromtiff($image); }
        elseif($extension=='TIF'){ $src = imagecreatefromtif($image); }        
        elseif($extension=='TIFF'){ $src = imagecreatefromtiff($image); }        
        elseif($extension=='bmp'){ $src = imagecreatefrombmp($image); }        
        elseif($extension=='BMP'){ $src = imagecreatefrombmp($image); }        
        elseif($extension=='eps'){ $src = imagecreatefromeps($image); }        
        elseif($extension=='EPS'){ $src = imagecreatefromeps($image); }        
        elseif($extension=='raw'){ $src = imagecreatefromraw($image); }        
        elseif($extension=='cr2'){ $src = imagecreatefromcr2($image); }        
        elseif($extension=='nef'){ $src = imagecreatefromnef($image); }         
        elseif($extension=='orf'){ $src = imagecreatefromorf($image); }          
        elseif($extension=='sr2'){ $src = imagecreatefromsr2($image); }
//        elseif($extension=='bmp'){ $src = imagecreatefrombmp($image); }
...
//    Used in signature common file as we have the condition that user has changed the signatures so will pass the offer id
    function getOfferPartnerProfile($partner = null,$offer_id,$getName='')
    {
	$offerPartner = App\Models\Access\User\User::where('id', $partner)->withTrashed()->with(['user_profile', 'business_profile', 'rentSignature'=>function($query)use($offer_id,$partner){$query->where('offer_id', $offer_id)->where('user_id', $partner)->first();},'signature' => function($query) use($offer_id,$partner) {
