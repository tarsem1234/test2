...
use App\Models\{Property,SaleOffer,RentOffer};
use Carbon\Carbon;
use Mail;
...
//	dd($previousDate);
        $properties = Property::where('created_at', '<', $previousDate)
                ...
                ->with(['rentOffer' => function($rentQuery) use($previousDate) {
                        $rentQuery->orderBy('created_at','desc')->first();
                    }, 'saleOffer' => function($saleQuery) use($previousDate) {
...
//		    dd($properties);
        foreach ($properties as $property) {
            ...
            } else if ($property->saleOffer->count() >= 1 && $property->rentOffer->count() == 0) {
...
//                    SaleOffer::where('id',$property->saleOffer->first()->id)->update(['status'=>config('constant.inverse_offer_status.rejected_by_seller')]);
            ...
            } else if ($property->saleOffer->count() == 0 && $property->rentOffer->count() >= 1) {
