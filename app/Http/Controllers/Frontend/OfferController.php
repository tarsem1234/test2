...
use Mail;
use App\Mail\Frontend\SendOffer;
use Log;
use Exception;
...
// Check property owner
	    ...
	    } else if ($data['type'] == config('constant.property_type.1') && Auth::id() != $existigProperty->user_id) {
...
//                $to            = $data['email'];
	...
	if ($offer->status == config('constant.inverse_rent_offer_status.rejected_by_landlord')) {
		...
		else if ($offer->tenant_signature == config('constant.offer_signature.true') && $offer->landlord_signature == config('constant.offer_signature.true')) {
		//if both parties have signed the documents show download button 
		$downloadButton = TRUE;
	    } else if ($offer->tenant_signature == config('constant.offer_signature.true') && $offer->landlord_signature == config('constant.offer_signature.false')) {
		...
		if ($offer->rentSignatures->isNotEmpty()) {
		    ...
		    } else if ($offer->buyer_id == Auth::id()) {
...
//                            dd($signature);
		...
		} else if ($offer->rent_counter_offers->isNotEmpty() && $offer->rent_counter_offers->first()->user_id != Auth::id()) {
...
//		    dd(Auth::id());
	...
	if ($offer->status == config('constant.inverse_offer_status.rejected_by_seller')) {
	    ...
	    } else if ($offer->buyer_signature == config('constant.offer_signature.true') && $offer->seller_signature == config('constant.offer_signature.true')) {
		//if both parties have signed the documents show download button 
		$downloadButton = TRUE;
	    } else if ($offer->buyer_signature == config('constant.offer_signature.true') && $offer->seller_signature == config('constant.offer_signature.false')) {
		...
		if ($offer->signatures->isNotEmpty()) {
		    ...
		    } else if ($offer->sender_id == Auth::id()) {
//                         if logged in user have not signed and is main buyer
...
//           $showContractToolsButton = TRUE;
		...
		} else if ($offer->sale_counter_offers->isNotEmpty() && $offer->sale_counter_offers->first()->user_id != Auth::id()) {
...
//           check is signature done on any of document corresponding to offer id ,then hide the contract button  
	    ...
	    } else if ($offer->owner_id == Auth::id() && $offer->status == config('constant.inverse_offer_status.pending') && $offer->sale_counter_offers->isNotEmpty() && $offer->sale_counter_offers->first()->user_id != Auth::id()) {
		$showNegotiationButton = TRUE;
	    }
	}
...
//                $message = "Please wait for Tenant's to review & sign the contract documents.";
		...
		//shoW offer accept reject negotiation
		$showOfferAceeptRejectButton = TRUE;
	    } else if ($offer->owner_id == Auth::id() && $offer->status == config('constant.inverse_rent_offer_status.pending') && $offer->rent_counter_offers->isNotEmpty() && $offer->rent_counter_offers->first()->user_id != Auth::id()) {
		$showNegotiationButton = TRUE;
	    }
	}
...
//                        $rentOffer->landlord->notify(new OfferShowsAcceptance($rentOffer->tenant));
			...
			if ($rentOffer->property_owner_user->hasRole(config('constant.inverse_user_type.Business'))) {
			    ...
			    } else if (Auth::id() == $rentOffer->buyer_id) {
...
//                dd($profile);
	...
	if (!empty($id) && !empty($propertyType)) {
	    ...
	    } else if ($propertyType == config('constant.inverse_property_type.Sale')) {
		$saleOffer = $this->_findSaleCounterOffer($id);
		if (!empty($saleOffer)) {
		    $status = config('constant.inverse_offer_status.rejected_by_buyer');
		    if ($saleOffer->owner_id == Auth::id()) {
