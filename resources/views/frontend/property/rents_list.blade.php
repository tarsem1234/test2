                    ...
                    <div class="right-dashboard-div-text">
                        <div class="row padding-space"> 
                            <div class="col-md-6 col-xs-12 col-sm-6 col-lg-6">
                                <h4>My Rent Listings</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-12 col-lg-6">
                                <a href="{{route('frontend.rentCreate')}}"><button class="btn btn-primary btn-lg" id="addlisting-btn"> Add New Listing </button></a>
                            </div>
                        </div>
                        <?php
                        if ($properties->count()) {
                                        ...
                                        <div class="listing-box">
                                            ...
                                            <div class="col-md-6 col-sm-12">
                                                ...
                                                <div class="listing-div-icons">
                                                    <span class="icon-listing-div">
                                                        <p><i class="fa fa-bed color-green" aria-hidden="true"></i>{{$property->architechture->beds??'NA'}}</p>
                                                    </span>
                                                    <span class="icon-listing-div">
                                                        <p><i class="fa fa-square color-black" aria-hidden="true"></i>{{$property->architechture->plot_size??'NA'}}</p>
                                                    </span>
                                                    <span class="icon-listing-div">
                                                        <p><i class="fa fa-bath color-black" aria-hidden="true"></i>{{ !empty($property->architechture->baths) ? config('constant.baths.'.$property->architechture->baths):'NA'}}</p>
                                                    </span>
                                                    <span class="icon-listing-div">
                                                        <p><i class="fa fa-usd color-green" aria-hidden="true"></i><span class="money1">{{round($property->price)??'NA'}}</span></p>
                                                    </span>
                                                </div>
                                                <table>
                                                    <tbody>
                                            ...
                                            <div class="col-md-12 col-sm-12 no-padding">
                                                <div class="text-center"> 
                                                    <a href="{{route('frontend.rentEdit',$property->id)}}" class="btn btn-default link-button5">EDIT</a>
                                                    ...
                                                    if (isset($property) && ($property->status == config('constant.inverse_property_status.Unavailable') || $property->status == config('constant.inverse_property_status.In Progress')) && Auth::check() && $property->user_id == Auth::id()) {
                                                        ?>
                                                            <a href="{{route('frontend.backToMarket',$property->id)}}" class="btn btn-default link-button5" id="">Activate Listing</a>
                                                    <?php } ?>
						   <?php if (!empty($property->propertyConditionDisclaimer)) {
                                                        ?>
                                                        <a href ="{{route('frontend.disclosureBySellerUpdate',['id'=>$property->id,'page'=>'rent_list'])}}" class="btn btn-default link-button5">EDIT DISCLOSURE</a>
                                                    <?php } else {
                                                        ?>
                                                        <a href="{{route('frontend.disclosureBySellerUpdate',['id'=>$property->id,'page'=>'rent_list'])}}" class="btn btn-default link-button5">ADD DISCLOSURE</a>
                                                    <?php } ?>    
						


                                                    <?php if(!empty($property->architechture->year_built) && $property->architechture->year_built!=null && $property->architechture->year_built <= 1978){
                                                        if($property->status != config('constant.property_status.inverse_property_status.Sold') && $property->lead_based==null && $property->lead_based_report==null && count($property->rentOffer) == 0){ ?>
                                                            <a href="{{route('frontend.leadBasedPaintHazardsRent',$property->id)}}" class="btn btn-default link-button1">ADD LEAD-BASED PAINT HAZARDS</a>
                                                        <?php } elseif($property->status != config('constant.property_status.inverse_property_status.Sold') && $property->lead_based != null && $property->lead_based_report!= null && count($property->rentOffer) == 0){ ?>
                                                            <a href="{{route('frontend.leadBasedPaintHazardsRent',$property->id)}}" class="btn btn-default link-button1">ADD LEAD-BASED PAINT HAZARDS</a>
                                                        <?php } elseif( count($property->rentOffer) == 0 && $property->status == config('constant.property_status.inverse_property_status.Sold') && $property->lead_based!= null && $property->lead_based_report!= null){ ?>
                                                        <!-- no button shown -->
                                                        <?php } ?>
                                                    <?php } ?>
                                                </div>
