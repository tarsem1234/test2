               ...
               <div class="para-text row">
                  <div class="form-group">
                     <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <label>Address: </label>
                            <input type="text" class="form-control" placeholder="Address" value="{{($offer->property->street_address??"") . ($offer->property->townhouse_apt? ', Apt# '.$offer->property->townhouse_apt :"") }}" readonly="readonly">
                            <!--<input type="text" class="form-control" placeholder="Address" value="{{ ($offer->property->street_address??"") . ($offer->property->townhouse_apt? ', Apt# '.$offer->property->townhouse_apt.' ,':"")}}" readonly="readonly">-->
                        </div>
                        <div class="form-group">
                            <label>City: </label>
                        ...
                        <div class="form-group">
                           ...
                           <input id="postal_code" class="form-control" min="0" value="<?php echo findZipCode($offer->property->zip_code_id); ?>" placeholder="Zip Code" type="number" readonly="readonly">
                        </div>
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        <h5>(d) PURCHASE PRICE:</h5> <p>$ <span class="text-readonly"><input type="text" class="readonly" id="text-form-input" value="{{ isset($purchasePrice)?$purchasePrice:$offer->tentative_offer_price }}" readonly="readonly"></span>
                           Dollars, to be paid in cash or equivalent good funds at closing.</p>
                     </div><!--col-md-12-->
                  </div>
                  <?php
//                  echo'<pre>';print_r($offer->sellerQuestionnaire);die;
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        <h5>(e) EARNEST MONEY:</h5><p> $<span class="text-readonly span_with_values">{{ isset($offer->sellerQuestionnaire->amount_for_earnest_money)?$offer->sellerQuestionnaire->amount_for_earnest_money:'NA' }}</span>
                           valid check or money order payable to Escrow Agent:  <span class="text-readonly span_with_values">{{ isset($offer->sellerQuestionnaire->where_funds_deposited)?$offer->sellerQuestionnaire->where_funds_deposited:'NA'}}</span>
                           whose address is:<span class="text-readonly span_with_values">{{ isset($offer->sellerQuestionnaire->legal_firm_address)?$offer->sellerQuestionnaire->legal_firm_address:'NA' }}</span>
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        ...
                        <textarea type="text" id="optional_message" class="form-control text-height" required="" readonly="readonly">{{$offer->sellerQuestionnaire->household_items??""}}</textarea>
                        <label>Items that are not included in the sale</label>
                        <input class="form-control" name="not_included_insale" type="text" value="{{$offer->saleAgreement->not_included_insale??""}}">
                        <label>Leased items</label>
                        <input class="form-control" name="leased_items" type="text" value="{{$offer->saleAgreement->leased_items??""}}">
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        ...
                        <p>Stipulation (A)</p>
                        <textarea type="text" rows="4" id="optional_message" class="form-control text-height" readonly="readonly">{{$offer->percentage_of_price?trans('strings.frontend.property.sale.stipulationPercentageA',['value'=>$offer->percentage_of_price]):($offer->fixed_amount?trans('strings.frontend.property.sale.stipulationFixedA',['value'=>$offer->fixed_amount]):"NA")}}</textarea>
                        <p>Stipulation (B)</p>
                        <textarea type="text" rows="4" id="optional_message" class="form-control text-height" readonly="readonly">{{$offer->sellerQuestionnaire->property_vacant_date?trans('strings.frontend.property.sale.stipulationB',['value'=>$offer->sellerQuestionnaire->property_vacant_date]):"NA"}}</textarea>
                        <p>Stipulation (C)</p>
                        <textarea type="text" rows="4" id="optional_message" class="form-control text-height" readonly="readonly">{{$offer->contingencies_explain??"NA"}}</textarea>
                     </div><!--col-md-12-->
                  </div>
                  <div class="form-group">
                     <div class="col-md-12">
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        <h5>(u) REAL ESTATE COMMISSIONS: </h5>
                        <p> REAL ESTATE COMMISSIONS: Seller authorizes closing company to debit Seller and pay commissions as follows at closing: Real Estate Firm Name:
                           <span class="text-readonly span_with_values">{{$offer->sellerQuestionnaire->real_estate_firm_name??"NA"}}</span>
                           will receive <span class="text-readonly span_with_values">{{$offer->sellerQuestionnaire->commission??"0"}}</span>
                           ...
                           . Other Real Estate Firm Name (if any):<span class="text-readonly"><input type="text" class="readonly" id="text-form-input" value="" readonly="readonly"></span>
                           will receive  <span class="text-readonly"><input type="text" class="readonly" id="text-form-input" value="" readonly="readonly"></span>
                           % of the purchase price. Other Licensee’s Name (if any) and Contact Information: <span class="text-readonly"><input type="text" class="readonly" id="text-form-input" value="" readonly="readonly"></span>.
                        </p>
                     </div><!--col-md-12-->
                  </div>
                  <?php
                  ...
                  <div class="form-group addenda_attach">
                     ...
                     <div class="col-md-12">
                        <div class="checkbox">
                            <input type="checkbox" class="styled-checkbox add_attach" disabled="disabled" <?php if (isset($addendas) && $addendas['0'] != '') {
                     ...
                     <div class="col-md-12 clearfix">
                        <div class="checkbox">
                           <input type="checkbox" class="styled-checkbox add_attach" name="addenda[]" id="Range" disabled="disabled" <?php
                           if (isset($addendas) && $addendas['0'] != '') {
                              foreach ($addendas as $addenda) {
                                 if ($addenda == config('constant.inverse_update_sale_agreement_by_seller.Tennessee Residential Property Condition Disclosure')) {
                                    ?>
                                           checked="checked"
                                           <?php
                                        }
                                     }
                                  } elseif(isset($offer->property->propertyConditionDisclaimer) && $offer->property->propertyConditionDisclaimer->count()){ ?>
                                           checked="checked"
                                  <?php } ?> value="{{config('constant.inverse_update_sale_agreement_by_seller.Tennessee Residential Property Condition Disclosure')}}">
                  ...
                  <div class="form-group">
                     <div class="col-md-12">
                        <h5>(y)  OFFER EXPIRATION DATE & TIME:  </h5><p> <span class="text-readonly">
                              <input type="text" class="readonly" id="text-form-input" name="offer_expiration" value="{{(isset($offer->buyerQuestionnaire->date_of_expiry) && $offer->buyerQuestionnaire->date_of_expiry) ? $offer->buyerQuestionnaire->date_of_expiry : date('Y-m-d', strtotime('+3 days', strtotime($offer->created_at)))}}" readonly="readonly"></span>
                  ...
                  <div class="btns-green-blue col-lg-12">
                     <button type="submit" class="button btn btn-blue">Submit</button>
                  </div><!--</btns-green-blue>-->
                  </form>
               </div>
            </div>
...
<script>
   $(document).ready(function () {
      ...
      $("form").submit(function (e) {
         if ($('.add_attach').is(":checked")) {
            $('.addenda_attach_error').hide();
         } else if ($('.add_attach').is(":not(:checked)")) {
            $('.addenda_attach_error').show();
            return false;
         }
      });
